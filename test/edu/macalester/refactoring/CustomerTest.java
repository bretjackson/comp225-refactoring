...
public class CustomerTest {
    ...
    public void basicChildrenRental() {
        ...
        assertEquals(customer.formatTextStatement(), expectedMessageFor("The Hulk", 1.5, 1.5, 1));
    }

    @Test
    public void shouldDiscountChildrensRentals() {
        customer.addRental(new Rental(THE_HULK, 4));
        assertEquals(customer.formatTextStatement(), expectedMessageFor("The Hulk", 3.0, 3.0, 1));
    }

    @Test
    public void basicNewReleaseRental() {
        customer.addRental(new Rental(IRON_MAN, 1));
        assertEquals(customer.formatTextStatement(), expectedMessageFor("Iron Man 4", 3.0, 3.0, 1));
    }

    @Test
    public void shouldNotDiscountNewReleaseRentalsButBonusFrequentRenterPoints() {
        customer.addRental(new Rental(IRON_MAN, 4));
        assertEquals(customer.formatTextStatement(), expectedMessageFor("Iron Man 4", 12.0, 12.0, 2));
    }

    @Test
    public void basicRegularRental() {
        customer.addRental(new Rental(SPIDER_MAN, 2));
        assertEquals(customer.formatTextStatement(), expectedMessageFor("Spiderman", 2.0, 2.0, 1));
    }

    @Test
    public void shouldDiscountRegularRental() {
        customer.addRental(new Rental(SPIDER_MAN, 4));
        assertEquals(customer.formatTextStatement(), expectedMessageFor("Spiderman", 5.0, 5.0, 1));
    }
    ...
    public void shouldSumVariousRentals() {
        ...
        assertEquals(customer.formatTextStatement(), "Rental record for fred\n\tThe Hulk\t1.5\n\tSpiderman\t2.0\n\tIron Man 4\t9.0\nAmount owed is 12.5\nYou earned 4 frequent renter points");
    }
